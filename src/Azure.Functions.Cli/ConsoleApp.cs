...
namespace Azure.Functions.Cli
{
    class ConsoleApp
    {
        ...
        public static async Task RunAsync<T>(string[] args, IContainer container)
        {
            ...
            try
            {
                // Setup the telemtry information
                app.tc.Context.User.Id = GetTelemetryUserID();
                app.tc.Context.Device.OperatingSystem = Environment.OSVersion.ToString();
                ...
                if (action != null)
                {
                    ...
                    {
                        LogEventIfAllowed(app.tc, app._consoleEvent);
                    }
                }
            }
            catch (Exception ex)
